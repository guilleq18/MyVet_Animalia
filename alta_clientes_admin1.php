...
while ($User=$ResUsuarios->fetch()) {
	if ($_POST['dni']==$User['dni']) {
		...
		header("location: alta_clientes.admin.php?f=2");
	}
...
if (strlen($contrasena)<8) {
	header("Location: alta_clientes.admin.php?f=3");
}else{
...
if (strcmp($contrasena, $contrasena2) === 0){
	...
	header("Location: alta_clientes.admin.php?f=4");
...
$permiso="g";
$permisos="INSERT INTO permisos (id_clientes, aplicaciones_cod, permisosperf) VALUE (:id, :app, :per)";
$resPermisos=$base-> prepare ($permisos);
$resPermisos->execute(array(":id"=>$idCliente['id_clientes'], ":app"=>$aplicaciones, ":per"=>$permiso));

header('Location: alta_clientes.admin.php?f=1');
